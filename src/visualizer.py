...
def draw(grid, robots=None, title="Workspace", ax=None, show=True):
    ...
    if robots:
        ...
        for robot in robots:
            ...
            # Robot body
            rect = patches.Rectangle(
                (c, r), n, n, linewidth=2, edgecolor="white", facecolor=color, alpha=0.85, zorder=3
            )
            ax.add_patch(rect)
...
def draw_sequence(grid, snapshots, titles=None, cols_per_row=5, save_path=None):
    ...
    fig, axes = plt.subplots(
        n_rows,
        n_cols,
        figsize=(n_cols * (grid.cols * 0.55 + 0.5), n_rows * (grid.rows * 0.55 + 0.5)),
    )
...
if __name__ == "__main__":
    ...
    snap(a, b, "3: A right")
    a.col += 1
    snap(a, b, "4: A right  [sw2]")
    b.row += 1
    snap(a, b, "5: B down")
    b.col -= 1
    snap(a, b, "6: B left  [sw3]")
