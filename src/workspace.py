...
class Workspace:
    ...
    def robots_overlap(self, row_a: int, col_a: int, n_a: int,
                             row_b: int, col_b: int, n_b: int) -> bool:
        """
        Would two robots overlap?
        Each robot has its own size (n_a, n_b) — they don't have to match.
        """
        return not (
            row_a + n_a <= row_b or row_b + n_b <= row_a or
            col_a + n_a <= col_b or col_b + n_b <= col_a
        )
    ...
    def can_move(self, robot: Robot, direction: str) -> bool:
        ...
        if self.robots_overlap(new_row, new_col, robot.n,
                               other.row, other.col, other.n):
            return False
    ...
    def get_state(self) -> "State":
        ...
        return State(
            pos_a   = self.robot_a.position(),
            pos_b   = self.robot_b.position(),
    ...
    def __repr__(self):
        return (f"Workspace(grid={self.grid}, "
