...
## Corpus Check
- 37 files · ~24,277 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 372 nodes · 927 edges · 26 communities detected
- Extraction: 48% EXTRACTED · 52% INFERRED · 0% AMBIGUOUS · INFERRED: 481 edges (avg confidence: 0.59)
...
Cohesion: 0.04
Nodes (62): all_adjacent_placements(), all_touching_placements(), blocks_overlap(), build_transform_tables(), canonical_key(), Canonicalizer, is_label_swap_symmetric(), pick_central_placements() (+54 more)
...
Cohesion: 0.07
Nodes (28): data_path(), get_data_dir(), get_plots_dir(), get_project_root(), get_src_dir(), get_testcases_dir(), plots_path(), directories.py -------- Central place for all project directory paths.  Ever (+20 more)

### Community 3 - "Community 3"
Cohesion: 0.19
Nodes (24): run_tests.py ------------ Discovers all test cases in testcases/ and runs them., simplify.py ----------- Reusable workspace-simplification pass.  Given a solved, Like the peak keeper, but on each per-face edge also keep enough cells     that, Fully-wall rows/cols to peel from each side: (top, bottom, left, right).     The, Wall-removal simplification driven by the contact heatmap.      All black walls, Run the simplification pass; save results into <plot_dir>/simplified/.     Retur, Count wall contact at every step, for both robots — including the     initial pl, Touched-wall cells to KEEP when thinning each side down to its peak.      Split (+16 more)

### Community 4 - "Community 4"
Cohesion: 0.16
Nodes (19): _clear_caches(), filter_excluded(), main(), Drop any file whose path contains one of the exclude folder names., main(), Drop cached entries but preserve lifetime hit/miss/eviction counters.          T, _compute_contact(), _compute_contact_at() (+11 more)
...
Cohesion: 0.14
Nodes (13): 2x2_robot_holes.py ------------------- Test case for a 2x2 robot scenario with, TwoByTwoHoles, 2x2_robot_no_holes.py ------------------- Test case for a 2x2 robot scenario w, TwoByTwoNoHoles, 3x3_robot_no_holes.py ------------------- Test case for a 3x3 robot scenario w, ThreeByThreeNoHoles, FourByFourHoles, 4x4_robot_holes.py ------------------- Test case for a 4x4 robot scenario with (+5 more)

### Community 7 - "Community 7"
Cohesion: 0.14
Nodes (10): FourByFourNoHoles, 4x4_robot_no_holes.py ------------------- Test case for a 4x4 robot scenario w, demo_validator.py ----------------- Manual demo for the validator., An n×n square robot on a grid.     Position is the (row, col) of its top-left c, Robot, _build_workspace_from_tiles(), Fresh Workspace with the given tile layout, robots at ref_ws's start., Restore robot positions from a State snapshot.         BFS uses this to backtra (+2 more)

### Community 8 - "Community 8"
Cohesion: 0.18
Nodes (7): plot_proof(), Solve `ws_template`, validate the path, and render the solution sequence     in, from_free_cells(), Would robot fit at (row, col) without hitting any obstacle or wall?         Doe, Can `robot` move one step in `direction`?         Checks: grid fit + no collisi, Move `robot` one step in `direction` if valid.         Returns True if move was, The set of free (non-obstacle) cells in this workspace's grid.
...
- **Thin community `Community 15`** (2 nodes): `.get_boundaries()`, `Return set of all boundary cell positions (row, col).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 16`** (2 nodes): `grid.py ------- The physical environment — a 2D map of free tiles and obstacle`, `grid.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 17`** (2 nodes): `.get_holes()`, `Return set of all hole cell positions (row, col).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 18`** (2 nodes): `.get_all_obstacles()`, `Return set of all obstacle positions — both holes and boundaries.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 19`** (2 nodes): `.display()`, `Print the raw grid. '.' = free, '#' = boundary, 'O' = hole.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 20`** (2 nodes): `All (row, col) tiles this robot occupies.`, `.cells()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 21`** (2 nodes): `robot.py -------- A single n×n square robot.  Knows only:   - its label`, `robot.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `Workspace` connect `Community 3` to `Community 0`, `Community 1`, `Community 2`, `Community 5`, `Community 6`, `Community 7`, `Community 8`, `Community 11`, `Community 13`?**
  _High betweenness centrality (0.310) - this node is a cross-community bridge._
- **Why does `Grid` connect `Community 5` to `Community 1`, `Community 2`, `Community 3`, `Community 6`, `Community 7`, `Community 8`, `Community 9`, `Community 11`, `Community 15`, `Community 16`, `Community 17`, `Community 18`, `Community 19`?**
  _High betweenness centrality (0.175) - this node is a cross-community bridge._
- **Why does `Robot` connect `Community 7` to `Community 1`, `Community 2`, `Community 3`, `Community 4`, `Community 5`, `Community 6`, `Community 8`, `Community 11`, `Community 20`, `Community 21`, `Community 22`, `Community 23`?**
