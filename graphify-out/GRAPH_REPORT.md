...
## Corpus Check
- 34 files · ~19,873 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 281 nodes · 680 edges · 27 communities detected
...
Cohesion: 0.11
Nodes (30): configure_caches_for_grid(), Resize module-level LRU caches so total memory scales with grid area.      Each, all_touching_placements(), _build_cell_bits(), _build_workspace(), _canonical_key(), _dedup_placements(), dig_search() (+22 more)

### Community 2 - "Community 2"
Cohesion: 0.15
Nodes (25): demo_validator.py ----------------- Manual demo for the validator., find_hardest_workspace.py ------------------------- Self-contained, optimized, Incrementally update valid-block-positions after digging one cell.     Only top, Precompute, per cell, the bit-position values under each transform., Build and set module-level transform tables. Called once in main process., Assign pre-built transform tables in worker processes (no recomputation).     A, Canonical key from a precomputed transforms_free tuple and per-transform     ti, All non-overlapping A/B placements where the two n*n blocks touch —     full ed (+17 more)

### Community 3 - "Community 3"
Cohesion: 0.11
Nodes (15): all_adjacent_placements(), _dig_options_n_strip(), Worker function: rebuild workspace and run solver.     Used by the batch-parall, Yield 1×n or n×1 strips that extend the valid region by one robot step.      F, Full-edge-adjacent pairs only: B directly right of or below A., _solve_payload(), discover_test_cases(), run_tests.py ------------ Discovers all test cases in testcases/ and runs them (+7 more)
...
Cohesion: 0.21
Nodes (7): 3x3_robot_holes.py ------------------- Test case for a 3x3 robot scenario with, ThreeByThreeHoles, Grid, 2D grid of tiles.       0 = free       1 = boundary (perimeter wall)      -1, Set a rectangle of cells as internal holes/islands., Set a rectangle of cells as boundary — same as add_hole but marks as BOUNDARY., Draw the full perimeter of the grid as boundary.

### Community 7 - "Community 7"
Cohesion: 0.17
Nodes (11): OneByOneNoHoles, 1x1_robot_no_holes.py ------------------- Trivial 1x1 swap with a pocket., 2x2_robot_holes.py ------------------- Test case for a 2x2 robot scenario with, TwoByTwoHoles, FourByFourHoles, 4x4_robot_holes.py ------------------- Test case for a 4x4 robot scenario with, FourByFourNoHoles, 4x4_robot_no_holes.py ------------------- Test case for a 4x4 robot scenario w (+3 more)
...
Cohesion: 1.0
Nodes (1): workspace.py ------------ The grid + both robots + all movement rules.  Owns

### Community 24 - "Community 24"
Cohesion: 1.0
Nodes (1): Restore robot positions from a State snapshot.         BFS uses this to backtra
...
- **Thin community `Community 15`** (2 nodes): `.get_holes()`, `Return set of all hole cell positions (row, col).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 16`** (2 nodes): `.get_boundaries()`, `Return set of all boundary cell positions (row, col).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 17`** (2 nodes): `.get_all_obstacles()`, `Return set of all obstacle positions — both holes and boundaries.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 18`** (2 nodes): `.display()`, `Print the raw grid. '.' = free, '#' = boundary, 'O' = hole.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 19`** (2 nodes): `label : name, e.g. 'A' or 'B'         n     : edge length in tiles         row`, `.__init__()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 20`** (2 nodes): `All (row, col) tiles this robot occupies.`, `.cells()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 21`** (2 nodes): `Return an independent copy of this robot.`, `.clone()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 22`** (2 nodes): `robot.py -------- A single n×n square robot.  Knows only:   - its label`, `robot.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 23`** (2 nodes): `workspace.py`, `workspace.py ------------ The grid + both robots + all movement rules.  Owns`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 24`** (2 nodes): `Restore robot positions from a State snapshot.         BFS uses this to backtra`, `.load_state()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 25`** (1 nodes): `demo_solver.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `Grid` connect `Community 6` to `Community 0`, `Community 1`, `Community 2`, `Community 3`, `Community 7`, `Community 8`, `Community 9`, `Community 11`, `Community 12`, `Community 13`, `Community 14`, `Community 15`, `Community 16`, `Community 17`, `Community 18`, `Community 23`, `Community 24`?**
  _High betweenness centrality (0.247) - this node is a cross-community bridge._
- **Why does `Robot` connect `Community 2` to `Community 0`, `Community 1`, `Community 3`, `Community 4`, `Community 6`, `Community 7`, `Community 8`, `Community 9`, `Community 11`, `Community 12`, `Community 13`, `Community 19`, `Community 20`, `Community 21`, `Community 22`, `Community 23`, `Community 24`?**
  _High betweenness centrality (0.203) - this node is a cross-community bridge._
- **Why does `Workspace` connect `Community 2` to `Community 0`, `Community 1`, `Community 3`, `Community 6`, `Community 7`, `Community 8`, `Community 9`, `Community 11`, `Community 12`, `Community 13`, `Community 23`, `Community 24`?**
